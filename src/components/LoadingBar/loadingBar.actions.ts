import Store from "./loadingBar.types";
import { random } from "lodash";

let interval: NodeJS.Timeout;

export const start = (store: Store) => {
    const { progress, count } = store.state;
    store.setState({ progress: count === 0 ? random(20, 30, false) : progress, count: count + 1 });
    if (count === 0) {
...
export const complete = (store: Store) => {
    const { progress, count } = store.state;
    const updatedCount = count - 1 <= 0 ? 0 : count - 1;
    if (!!interval) {
        clearInterval(interval)
    }
    store.setState({ progress: updatedCount === 0 ? 100 : progress, count: updatedCount });
};